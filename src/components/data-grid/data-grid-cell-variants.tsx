      ...
      if (filesToValidate.length > 0) {
        if (!skipUpload) {
          ...
          const finalFiles = filesWithTemp
            ...
            .filter((f) => f.url !== undefined) as FileCellData[];
...
export function CustomCell<TData>({
  ...
  return (
    ...
    >
      <div
        data-slot="grid-cell-content"
        className={cn("size-full overflow-hidden flex items-center", {
          "px-1.5": !(cell.column.columnDef.meta?.cell?.variant === "custom" && cell.column.columnDef.meta?.cell?.noPadding),
        })}
      >
