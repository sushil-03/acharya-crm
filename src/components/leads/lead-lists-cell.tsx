...
import { useGetLeadLists, type LeadListMembership } from "@/components/lists/hook/query/use-get-lead-lists";
...
export function LeadListsCell({ leadId, leadName }: LeadListsCellProps) {
  const { data: memberships, isLoading } = useGetLeadLists(leadId);
  const [open, setOpen] = useState(false);

  return (
    <div className="flex items-center gap-1 flex-wrap group min-w-0">
      {isLoading && <span className="text-muted-foreground text-xs">...</span>}
      {!isLoading &&
        memberships?.map((m: LeadListMembership) => (
          <span
            key={m.id}
      ...
      <AddToListDialog open={open} onOpenChange={setOpen} leadId={leadId} leadName={leadName} />
    </div>
  );
}
